...
function handleSave(payload) {
  ...
  // 1. Generate contract number if not set
  var contractNumber = d.contractNumber;
  var currentYear = new Date().getFullYear();
  if (!contractNumber || contractNumber.trim() === "") {
    var y = parseInt(numbering.year) || currentYear;
  ...
  // 2. Upload file to Drive if present
  if (fileInfo && fileInfo.base64 && fileInfo.fileName) {
    try {
      // Clean contract number for file name
      var safeContractNum = contractNumber.replace(/\//g, "-");
      var newFileName = safeContractNum + "_" + fileInfo.fileName;
      
      // Get folder config
      var config = handleGetPublicConfig();
      var rootFolderId = config.DRIVE_ROOT_FOLDER_ID;
      var rootFolder = null;
      ...
      if (rootFolderId) {
        try {
          ...
          console.warn("Invalid root folder ID: " + rootFolderId);
        }
      }
      if (!rootFolder) {
      ...
      var yearFolderName = String(currentYear);
      var subFolders = rootFolder.searchFolders("title = '" + yearFolderName + "'");
      var targetFolder = null;
      
      if (subFolders.hasNext()) {
        targetFolder = subFolders.next();
      } else {
        targetFolder = rootFolder.createFolder(yearFolderName);
      }
