...

{
    ...
    /**
     * Cache for the consultant prompt
     */
    _cachedConsultantPrompt: null,

    /**
     * Fetch System prompt for AI Consultant from external MD file
     */
    _getConsultantSystemInstruction: async () => {
        if (gemini._cachedConsultantPrompt) {
            return gemini._cachedConsultantPrompt;
        }

        try {
            // Append a timestamp to bypass browser cache during dev if needed, or rely on normal caching
            const res = await fetch('prompts/consultant_prompt.md', { cache: 'no-cache' });
            if (!res.ok) throw new Error("File prompt không tồn tại");

            const promptText = await res.text();
            gemini._cachedConsultantPrompt = promptText;
            return promptText;
        } catch (error) {
            console.error('Lỗi lấy file prompt:', error);
            // Fallback prompt in case the file couldn't be loaded
            return "Bạn là Chuyên gia phân tích hợp đồng. Vui lòng rà soát rủi ro pháp lý và tài chính cho văn bản này.";
        }
    },
    ...
    extractFile: async (file) => {
        try {
            if (file.name.endsWith('.docx')) {
                ...
                // Save text for later use in consultant to save parsing time again
                formManager.currentDocText = text;
    ...
    /**
     * Consultant Entry: Generate Markdown Review of Contract
     */
    consultContract: async (file, preLoadedText) => {
        try {
            const url = `https://generativelanguage.googleapis.com/v1beta/models/${CONFIG.GEMINI_MODEL}:generateContent?key=${CONFIG.GEMINI_API_KEY}`;
            ...
            if (preLoadedText) {
                contents = [{
                    role: "user",
                    parts: [{ text: `Dưới đây là văn bản hợp đồng:\n${preLoadedText}` }]
                }];
            } else if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
                const { base64, mimeType } = await fileManager.toBase64(file);
                contents = [{
                    role: "user",
                    parts: [
                        { inlineData: { data: base64, mimeType: mimeType } },
                        { text: "Hãy đọc văn bản trên." }
                    ]
                }];
            } else {
                throw new Error("Không tìm thấy nội dung văn bản để tư vấn.");
            }

            const systemInstructionText = await gemini._getConsultantSystemInstruction();
            ...
            const payload = {
                ...
                contents: contents,
                generationConfig: {
                    temperature: 0.3, // Slightly higher for varied reasoning 
            ...
            ui.toast('Lỗi AI Tư Vấn', error.message || 'Không thể lấy tư vấn từ AI.', 'error', 10000);
            return null;
        }
    },
